...
`Dsaga\Admin\Widgets\Box`用来生成box组件：
...
`Dsaga\Admin\Widgets\Collapse`类用来生成折叠插件：
```php
...
`Dsaga\Admin\Widgets\Form`类用来快速构建表单：
...
`Dsaga\Admin\Widgets\InfoBox`类用来生成信息展示块：
...
`Dsaga\Admin\Widgets\Tab`类用来生成选项卡组件：
...
`Dsaga\Admin\Widgets\Table`类用来生成表格：
